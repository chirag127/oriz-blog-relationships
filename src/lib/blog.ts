...
/**
 * Content verticals — collapse raw category strings into the five editorial
 * "desks" this relationships letter covers. Drives the marker colour (--v-*)
 * on rows and desk kickers so each desk reads differently at a glance.
 *
 * Internal ids are kept generic (dev/finance/culture/health/personal) so the
 * shared components + colour tokens compile unchanged; only the labels + the
 * category→desk mapping are specific to Between.
 *   dev → Connect · finance → Repair · culture → Distance
 *   health → Boundaries · personal → Self
 */
...
/** Map a raw category string → one of the five desks. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(boundar|no|limit|space|respect|guilt)/.test(c)) return 'health'
  if (/(repair|conflict|fight|argument|apolog|trust|rupture)/.test(c)) return 'finance'
  if (/(distance|long-distance|apart|away|reunion|move)/.test(c)) return 'culture'
  if (/(self|solo|single|grow|worth|attach)/.test(c)) return 'personal'
  // default desk — communication / connection
  return 'dev'
}
